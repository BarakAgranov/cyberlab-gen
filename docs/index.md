...
- **`validation.md`** (~470 lines) — the validator's mechanical layers (four active in v1; Layer 4 real-platform apply is v2-deferred but its section is preserved for design continuity), report shape, refinement loop integration. Entry points: `§6.3` (the five layers at a glance, with v1 vs. v2 status), `§6.4`–`§6.8` (one section per layer; Layer 4 is `§6.7`), `§6.10` (refinement loop integration).
...
| What gets validated at each layer? | `validation.md §6.4` (Layer 1) through `§6.8` (Layer 5). `§6.3` is the at-a-glance summary; v1 runs four layers (1, 2, 3, 5) — Layer 4 is v2-deferred but its section is preserved for design continuity. |
