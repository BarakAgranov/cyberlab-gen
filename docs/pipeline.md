...
**MITRE and CVE validation.** Technique IDs are validated against the bundled MITRE ATT&CK reference; CVE references are validated against NVD via the external_data_sources registry. If a hallucinated ID surfaces, the stage rejects the output and re-prompts the Extractor with the specific hallucinated ID flagged. Counts against the stage's retry budget.
...
**Responsibilities.** Consume the AttackSpec, decide which chain steps become phases, which become steps within phases, which become lab resources, which become manual prerequisites. Apply the preference ordering from `schema.md §4.20` at the step level: each step carries its own reproducibility classification. Phase shape emerges from the mix of step classifications. Declare which facets the lab uses — `runtime:*` (lab-derived; Planner is the sole authority) and lab-derived `lab_class_signal:*` (`simulated_components`, `multi_language`, `parameterized`, etc.). `target:*` and blog-derived `lab_class_signal:*` are inherited from the AttackSpec (where the Extractor authored them).
