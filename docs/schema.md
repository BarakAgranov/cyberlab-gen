...
**Detection enums (component, severity, format) are closed in v1.** Extended only by maintainer PR, not by agent proposal at runtime. This is intentional: these enums reflect industry-stable categories, and runtime additions would create more noise than value. Agent proposals are reserved for the open-set registries (value_types, facets, external_data_sources, execution_contexts).
