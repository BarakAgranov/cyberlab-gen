...
4. Implement `StaticCatalogsRegistry`. The type split between `ExternalDataSourceEntry` and `StaticCatalogEntry` is enforced structurally — `StaticCatalogEntry` does not declare `enrichment_triggers`, `discrepancy_materiality_rules`, or `notes_for_extractor`, and `extra="forbid"` (inherited from `ArtifactModel`) rejects them at Layer 1. Per `schema.md §4.14`, this split keeps each registry's semantic role clean. Tests must pin the structural guarantee explicitly with construct-and-reject patterns (e.g., constructing a `StaticCatalogEntry` with `enrichment_triggers` should raise `ValidationError`).
5. Tests in `tests/unit/schemas/test_registries.py`: every Pydantic model parses an empty `entries:` list; the static-catalogs entry rejects external-data-sources-only fields via structural enforcement; the overlay file shape's `_proposal_keys_match_entries` validator catches mismatches.
