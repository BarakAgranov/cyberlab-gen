...
def _as_dict(value: object) -> dict[str, object]:
    ...
    NVD JSON is parsed as ``object``; ``isinstance(x, dict)`` alone narrows to
    ``dict[Unknown, Unknown]`` under pyright strict, which then poisons every
    downstream ``.get``. Building a fresh ``dict[str, object]`` keeps the parser
    fully typed without per-line ``# pyright: ignore`` noise.
    """
