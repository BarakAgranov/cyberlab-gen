...
Phase 1 — the front half of the pipeline runs for real against a paid provider. The `extract` verb works end-to-end: Ingestion → Extractor → Layer-1 static-schema validation → Extractor-Jury → persistence, with per-call cost recording, the catastrophe ceiling, the post-Extractor interactive interrupt, and the propose→overlay→validate proposal loop (ADR 0044). Persistence is the run-store, which saves every run on every exit path (ADR 0039); the agent layer runs on pydantic-ai (ADR 0036) and observability is local Phoenix (ADR 0041).

Phase 2 — the plan slice runs end-to-end. The `plan` verb works: `cyberlab-gen plan <attack-spec.yaml>` runs Planner → Planner-Jury → the semantic cross-check (the second mechanical validation layer) → persistence, producing a **Pydantic-structurally-valid + Layer-2-(semantic-cross-check)-valid** `lab.yaml` (ADR 0097). **Caveat (ADR 0099 §6):** manifest **Layer-1 registry-membership** validation is **not wired** into the plan pipeline — `StaticSchemaValidator` (the `UNKNOWN_FACET` / value-type membership check) runs only on the AttackSpec in `extract`, never on the manifest, and the cross-check skips unknown facets by design. So a `plan`-produced `lab.yaml` can declare a facet/value-type that resolves in no registry (a typo, or a Planner-proposed-but-unpromoted facet) and still ship; wiring the membership check is an owned deferral to the Phase-3 `generate` Validator stage (provisional `PendingProposals` resolution must land with it). The Planner **proposes** `runtime:*`/lab-derived `lab_class_signal:*` facets; the post-Planner interactive interrupt (Task 8, ADR 0100) — the LabManifest four-option menu + per-proposal Accept/Edit — reviews them and promotes accepted ones to a **run-scoped** overlay (`<run_dir>/registry-overlay`), never the shared production vocabulary (`plan` is a dev/eval command, ADR 0096; real production promotion is `generate`'s job, Phase 3+). `plan` is now **interactive by default with an `--auto` bypass** (mirrors `extract`). **Callable agents today: the Extractor, the Extractor-Jury, the Planner, and the Planner-Jury.**
